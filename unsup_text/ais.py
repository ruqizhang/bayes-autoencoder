...
class AIS(object):
    def __init__(self, model, dataset, optimizer = None,
                 num_samples = 5, num_beta = 500, nprint = 25):
    ...
    def ais_step(self, schedule, log_prob, input_data, backwards = False):
        ...
        data_len = len(input_data)/64.0
        #print(self.num_beta/data_len)
        max_epochs = math.ceil(self.num_beta/data_len)
        ...
        while b < (self.num_beta-2):
            ...
            count=0
            for batch, i in enumerate(range(0, input_data.size(0) - 1, 35)):
