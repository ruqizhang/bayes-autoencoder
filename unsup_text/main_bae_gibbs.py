...
def evaluate(data_source):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        ...
        # xi = xi.squeeze()
        # xi_dim = xi.size(1)
        # q_z = 1/((2*np.pi)**(xi_dim/2.0))*(torch.sum(xi*xi,1).mul(-0.5).exp())
        # log_q_z = np.linalg.det(jacobian.cpu().numpy())
        # z_entropy = q_z.data*torch.from_numpy(log_q_z).cuda(device_id)
        # z_entropy = torch.sum(z_entropy)
        loss = BCE
